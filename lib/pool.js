...
var cryptoNightLite = multiHashing['cryptonight_lite'];
...
Miner.prototype = {
    ...
    getTargetHex: function(){
        ...
        var buffArray = Array.prototype.slice.call(buff, 0).reverse();
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        if (majorVersion >= 4) {
            ...
            // CN
            hash = cryptoNight(convertedBlob, 0);
        }
        shareType = 'valid';
    }
    ...
    if (hash.toString('hex') !== resultHash) {
        ...
        return [ false, `Wrong hashing algorithm` ];
    }

    var hashArray = Array.prototype.slice.call(hash, 0).reverse();
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
	    ...
	    if (!noncePattern.test(params.nonce)) {
            ...
            var [ shareAccepted, reason ] = processShare(miner, job, blockTemplate, params.nonce, params.result);
...
function startPoolServerTcp(callback){
    async.each(config.poolServer.ports, function(portData, cback){
        ...
        const server = net.createServer(function(socket){
            ...
            socket.setTimeout(3600 * 10 * 1000);
            ...
            socket.on('data', function(d){
                dataBuffer += d;
                if (Buffer.byteLength(dataBuffer, 'utf8') > 512000){ //500KB
                    dataBuffer = null;
        ...
        server.on('error', function(err){
            log('warn', logSystem, 'Server error %j', [err]);
        });
        
        server.listen(portData.port, function (error, result) {
            if (error) {
